...
class inPaintingLoss(nn.Module):
    ...
    def forward(self, input, mask, output, gt):
        ...
        if output.shape[1] == 3:
            ...
            output_features = self.extractor(output_features)
        ...
        for i in range(3):
            losses['style_out'] += self.l1(gram_matrix(output[i]),gram_matrix(gt[i]))
            losses['style_comp'] += self.l1(gram_matrix(computed_output[i]),gram_matrix(gt[i]))
